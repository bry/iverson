...
class TestIntToRounded < Test::Unit::TestCase
  ...
  def test_result_format_random_number_range_to_rounded_s
    ...
    for i in 0..max_iterations
      ...
      valid_rounded_s = case temp_x
        when 0 then /\$[0-9]{0,3}[KMBT]{0}/ 
        when 0...1000 then /\$[0-9]{0,3}[KMBT]{0}/ 
        when 1000...1000000 then /\$[0-9]{0,3}\.{0,1}[0-9]{0,1}[K]{1}/ 
        when 1000000...1000000000 then /\$[0-9]{0,3}\.{0,1}[0-9]{0,1}[M]{1}/ 
        when 1000000000...1000000000000 then /\$[0-9]{0,3}\.{0,1}[0-9]{0,1}[B]{1}/ 
        when 1000000000000...1000000000000000 then /\$[0-9]{0,3}\.{0,1}[0-9]{0,1}[T]{1}/ 
